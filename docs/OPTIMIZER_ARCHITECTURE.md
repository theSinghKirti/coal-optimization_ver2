...
- Rakes are whole numbers end-to-end here too: `applyOptimizedToPlant()`
  converts the fractional share-space result into whole rakes by largest
  remainder, conserving each plant's total exactly (fractional rake values
  are never stored in dashboard state; the dashboard UI rejects fractional
  rake entries with a clear message instead of rounding them).

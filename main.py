...
Rakes are strictly whole numbers everywhere: the Excel reader, the daily
variation CSV, the --freeze values and the manual-change test all REJECT
fractional (and negative) rake quantities with a clear error - rakes are
physical units and are never rounded or truncated.
...
# ---------------------------------------------------------------
def is_whole_rake(value):
    """Rakes are physical units: only non-negative whole numbers are valid.
    Returns True for e.g. 0, 1, 52; False for 10.5, -1, NaN, inf."""
    try:
        num = float(value)
    except (TypeError, ValueError):
        return False
    if not (num == num) or num in (float("inf"), float("-inf")):  # NaN / infinity
        return False
...
def clean_and_validate(df):
    ...
    invalid = df[~df["Rakes"].apply(is_whole_rake)]
    if not invalid.empty:
        raise ValueError(
            "Rake quantities must be non-negative WHOLE numbers - fractional "
            "or negative values are rejected, never rounded or truncated:\n"
            + invalid[["Plant", "Source", "Rakes"]].to_string(index=False)
        )
...
def manual_change_test(df, plant, source, new_rakes):
    ...
    if not is_whole_rake(new_rakes):
        raise ValueError(
            f"New rakes for {plant}/{source} must be a non-negative whole number "
            f"(got {new_rakes}) - fractional rakes are rejected, never rounded."
        )

    old_plant_summary, _ = compute_weighted_vc(df[df["Plant"] == plant])
    old_vc = old_plant_summary.loc[0, "Weighted VC"]

    df.loc[mask, "Rakes"] = int(new_rakes)
...
def apply_daily_variation(df, csv_path):
    ...
    Available rakes are physical rakes: only non-negative whole
    numbers are accepted - a fractional override is rejected, never
    ...
    for _, r in daily.iterrows():
        if not is_whole_rake(r["available_rakes"]):
            raise ValueError(
                f"FATAL: daily variation '{r['plant']}/{r['source']}' sets "
                f"available_rakes = {r['available_rakes']} - rakes must be "
                "non-negative whole numbers. Fractional or negative rakes are "
                "rejected, never rounded."
            )
        mask = (df["Plant"] == r["plant"]) & (df["Source"] == r["source"])
        if mask.any():
            df.loc[mask, "Available Rakes"] = int(r["available_rakes"])
        else:
...
def parse_freeze_arg(freeze_str):
    ...
    Frozen values are physical rakes: only non-negative whole numbers are
    accepted - a fractional freeze value (e.g. "Parichha:NCL=52.5") is
    rejected with a clear error, never rounded or truncated.
    """
    frozen = {}
    if not freeze_str:
        return frozen
    for chunk in freeze_str.split(","):
        ...
        value = value.strip()
        if not is_whole_rake(value):
            raise ValueError(
                f"FATAL: frozen rake value '{value}' for {plant.strip()}/{source.strip()} "
                "is not a non-negative whole number. Rakes are never fractional; "
                "rejecting instead of rounding."
            )
        frozen[(plant.strip(), source.strip())] = int(value)
    return frozen
...
def optimize_plant(df, plant, base_col="Rakes", frozen=None,
    ...
    # Input rakes are validated whole (clean_and_validate / apply_daily_variation),
    # so the plant total is a whole number - no rounding of user input here.
    total_rakes = int(sub[base_col].sum())
    ...
    for i, s in enumerate(sources):
        if (plant, s) in frozen:
            fixed = int(frozen[(plant, s)])
            lower_bounds.append(fixed)
            upper_bounds.append(fixed)
        else:
            # min/max % bounds converted to whole-rake bounds: rounding a
            # computed percentage bound is legitimate (it is not a rake input).
    ...
    # Integer decision variables / greedy integers: solution values are whole
    # rakes. int(round(...)) only normalizes solver floating-point noise
    # (13.0000001 -> 13); fractional INPUT rakes were rejected at validation.
    optimized = [int(round(v)) if abs(v - round(v)) < 1e-6 else float(v) for v in optimized]
    ...
    for plant in opt_all["Plant"].unique():
        ...
        for _, r in sub.iterrows():
            export_rows.append({
                "Plant": r["Plant"],
                "Source": r["Source"],
                "Original Rakes": int(r["Rakes"]),
                "Daily Available Rakes": int(r["Available Rakes"]),
                "Optimized Rakes": int(r["Optimized Rakes"]),
                "Frozen": r["Frozen"],
                "Method": method,

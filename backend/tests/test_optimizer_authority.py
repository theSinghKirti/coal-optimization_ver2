...
def test_non_integer_rakes_returns_business_validation_error():
    """Fractional rake inputs are rejected outright by the Pydantic schema
    (HTTP 422) - never rounded, never truncated, never silently accepted."""
    bad_payload = {
        "plants": [{
            "plant": "TestPlant",
            "sources": [
    ...
    assert "whole number" in resp.json()["detail"][0]["msg"].lower()


def test_fractional_min_max_rakes_rejected_not_rounded():
    """minRakes/maxRakes must also be whole numbers - a fractional bound
    (e.g. 5.5) is rejected, never rounded to 6."""
    for field in ("minRakes", "maxRakes"):
        bad_payload = {
            "plants": [{
                "plant": "TestPlant",
                "sources": [
                    {"company": "TestCo", "current_rakes": 10, "current_vc": 4.0, "minRakes": 5.5, "maxRakes": 15},
                ],
            }]
        }
        if field == "maxRakes":
            bad_payload["plants"][0]["sources"][0]["maxRakes"] = 15.5
        resp = client.post("/optimize", json=bad_payload)
        assert resp.status_code == 422, f"{field} fractional bound not rejected"
        assert "whole number" in resp.json()["detail"][0]["msg"].lower()


def test_all_response_rake_fields_serialize_as_integers():
    """The API must emit rakes as JSON integers - current, optimized, delta,
    plant totals, and min/max bounds. Only VC values may be decimals."""
    resp = client.post("/optimize", json=VALID_PAYLOAD)
    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] in ("Optimal", "Feasible")
    ...
    for a in body["allocations"]:
        assert type(a["current_rakes"]) is int, a
        assert type(a["optimized_rakes"]) is int, a
        assert type(a["delta_rakes"]) is int, a
        assert type(a["minRakes"]) is int, a
        assert type(a["maxRakes"]) is int, a

    for p in body["plants"]:
        assert type(p["current_rakes"]) is int, p
        assert type(p["optimized_rakes"]) is int, p

    company_totals = defaultdict(int)
    for a in body["allocations"]:
        company_totals[a["company"]] += a["optimized_rakes"]
    assert all(type(v) is int for v in company_totals.values())

    total_after = sum(a["optimized_rakes"] for a in body["allocations"])
    assert type(total_after) is int

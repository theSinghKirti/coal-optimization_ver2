...
bounds, no RSD logic - and accepts fractional rakes (they are weighting
factors, not physical allocations).
"""
import math
...
def test_fractional_rakes_are_accepted():
    """Unlike /optimize (physical rake allocation, whole numbers only), the
    calculator weights averages and accepts fractional actual rakes."""
    payload = {
        "plants": [
            {
                "plant": "PlantA",
                "sources": [
                    {"company": "BCCL", "current_rakes": 12.3, "current_vc": 4.78, "rakes": 12.3},
                ],
            },
        ]
    }
    ...
    assert body["status"] == "ok"
    assert body["plants"][0]["actual_vc"] == pytest.approx(4.78)
    assert body["total_rakes_actual"] == pytest.approx(12.3)
...
def test_infinite_rakes_return_business_validation_error():
    """+inf passes the field constraint (it is >= 0) but is caught by the
    module's business validation."""
    payload = _payload()
    ...
    assert any(e["field"] == "rakes" for e in body["errors"])

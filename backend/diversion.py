...
Fractional rakes are accepted on purpose: unlike the optimizer - which
allocates physical rakes and therefore demands integer values - this
calculator only weights averages, so fractional input (e.g. a 12.3-rake
receipt) is mathematically well-defined.
...
def _validate(request: DiversionRequest) -> List[ValidationErrorEntry]:
    ...
    current_vc > 0 are enforced at parse time). Catches non-finite values."""
    ...
    for plant in request.plants:
        for src in plant.sources:
            ...
            if not math.isfinite(src.rakes) or src.rakes < 0:
                errors.append(ValidationErrorEntry(
                    plant=plant.plant, company=src.company, field="rakes",
                    message="Actually diverted rakes must be a non-negative number.",
                ))
    return errors
...
def calculate(request: DiversionRequest) -> DiversionResponse:
    ...
    for plant, triples in by_plant.items():
        ...
        plants_result.append(PlantDiversionResult(
            plant=plant,
            current_rakes=round(cur_total, 6),
            actual_rakes=round(act_total, 6),
    ...
    return DiversionResponse(
        ...
        total_rakes_current=round(sum(r[2] for r in rows), 6),
        total_rakes_actual=round(sum(r[4] for r in rows), 6),

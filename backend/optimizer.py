...
Rakes are strictly whole numbers end-to-end: every incoming rake quantity
(current_rakes, minRakes, maxRakes) must be a non-negative integer - the
Pydantic schemas reject fractional values (e.g. 10.5) outright, they are
never rounded or truncated - and every outgoing rake quantity
(current/optimized/delta rakes, plant/company/global totals) is an integer.
...
def _validate(request: OptimizeRequest) -> List[ValidationErrorEntry]:
    ...
    enforce (current_vc > 0, current_rakes/minRakes/maxRakes as non-negative
    WHOLE numbers with fractional values rejected at parse time). Catches
    things field constraints can't express, e.g. maxRakes < minRakes."""
    ...
    for plant in request.plants:
        for src in plant.sources:
            ...
            if src.minRakes < 0 or src.maxRakes < 0:
                errors.append(ValidationErrorEntry(
                    plant=plant.plant, company=src.company, field="minRakes",
                    message="Rake bounds must be non-negative whole numbers.",
                ))
...
def optimize(request: OptimizeRequest) -> OptimizeResponse:
    ...
    # ---- Company totals (must be conserved exactly). Every value is a whole
    # number here: current_rakes is validated as whole at parse time.
    ...
    plant_totals: Dict[str, int] = defaultdict(int)
    for plant, company, current_rakes, vc, minR, maxR in rows:
    ...
    def build_solver_with_core_constraints() -> Tuple["pywraplp.Solver", Dict[int, "pywraplp.Variable"]]:
        ...
        variables = {}
        for idx, (plant, company, current_rakes, vc, minR, maxR) in enumerate(rows):
            # minR/maxR are already validated whole numbers - never rounded here.
            lb = int(minR)
            ub = int(maxR)
            if ub < lb:
        ...
        for company, idxs in by_company.items():
            solver.Add(
                solver.Sum(variables[i] for i in idxs) == int(company_totals[company])
            )
    ...
    # Integer decision variables: the solver's solution values are whole
    # numbers. int(round(...)) here only normalizes floating-point noise
    # (e.g. 13.0000001 -> 13) - it can never turn a fractional input into a
    # whole output, because fractional inputs were rejected during validation.
    optimized_rakes_by_idx = {
        idx: int(round(variables2[idx].solution_value())) for idx in range(len(rows))
    }
    ...
    for company, idxs in by_company.items():
        ...
        constraint_status.append(ConstraintStatusEntry(
            ...
            satisfied=ok,
            detail=f"{company_totals[company]} -> {total_after}",
        ))
    ...
    for plant, idxs in by_plant.items():
        ...
        lower = int(round(current_total * request.plantMinPct))
        upper = int(round(current_total * request.plantMaxPct))
        ok = (total_after >= lower) and (total_after <= upper)
        constraint_status.append(ConstraintStatusEntry(
            name=f"Plant total bound: {plant}",
            satisfied=ok,
            detail=f"{total_after} within [{lower}, {upper}]",
        ))
    ...
    constraint_status.append(ConstraintStatusEntry(
        ...
        satisfied=total_before == total_after_all,
        detail=f"{total_before} -> {total_after_all}",
    ))
